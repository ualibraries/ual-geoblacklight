...
module ApplicationHelper
  ...
  def ual_link_to_previous_document(previous_document, classes: 'previous page-link', **addl_link_opts)
    link_opts = session_tracking_params(previous_document, search_session['counter'].to_i - 1).merge(class: classes,
                                                                                                     rel: 'prev').merge(addl_link_opts)
    link_to_unless previous_document.nil?, raw(t('views.pagination.previous')), url_for_document(previous_document),
                   link_opts do
  ...
  def ual_link_to_next_document(next_document, classes: 'next page-link', **addl_link_opts)
    link_opts = session_tracking_params(next_document, search_session['counter'].to_i + 1).merge(class: classes,
                                                                                                 rel: 'next').merge(addl_link_opts)
  ...
  def ual_link_back_to_catalog(opts = { label: nil })
    ...
    label ||= t('blacklight.back_to_bookmarks').html_safe if link_url =~ /bookmarks/
